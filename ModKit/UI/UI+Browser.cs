...
namespace ModKit {
    ...
    public static partial class UI {
        ...
        public class Browser<Definition, Item> : Browser {
            ...
            private ManualResetEvent _cancellationTokenSource;
            ...
            class DefinitionComparer : IComparer<Definition>
            {
                public class Pair
                {
                    ...
                    public Pair(IComparable x, IComparable y)
                    {
                        this.x = x;
                        this.y = y;
                    }
                ...
                private readonly Func<Definition, IComparable[]> sortKeys;

                public DefinitionComparer(Func<Definition, IComparable[]> sortKeys)
                {
                ...
                public int Compare(Definition x, Definition y)
                {
                    ...
                    int minLength = Math.Min(xKeys.Length, yKeys.Length);
                    var zipped = new List<Pair>();

                    for (int i = 0; i < minLength; i++)
                    {
                        var pair = new Pair(xKeys[i], yKeys[i]);
                        zipped.Add(pair);
                    }
                ...
                if (Event.current.type == EventType.Layout) {
                    ...
                    if (_finishedSearch || isSearching) {
                        ...
                        if ((cachedSearchResults.Count > 0 || nothingToSearch) && (_searchQueryChanged || _finishedSearch)) {
                            ...
                            var comparer = new DefinitionComparer(sortKeys);
                    ...
                    if (needsReloadData) {
                        ...
                        if (!isSearching) {
                            _cancellationTokenSource = new ManualResetEvent(false);
                            ThreadPool.QueueUserWorkItem(state => UpdateSearchResults(_searchText, definitions, searchKey, sortKeys, search));
            ...
            public void UpdateSearchResults(string searchTextParam,
                ...
                IEnumerable<string> items = searchTextParam.Split(' ').Select(s => s.ToLower());
                HashSet<string> terms = new HashSet<string>(items);
                if (search) {
                    foreach (var def in definitions) {
                        if (_cancellationTokenSource.WaitOne(0)) {
                            isSearching = false;
                            return;
                        }
