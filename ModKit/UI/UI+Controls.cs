    ...
    public static partial class UI {
        ...
        public static GUILayoutOption[] AddDefaults(this GUILayoutOption[] options, params GUILayoutOption[] desired) {
            ...
            if (desired.Length > 0)
                ...
                options = options.AddDefaults(AutoWidth()).ToArray();
            return options;
        }
        ...
        public static bool EditableLabel(ref string label, ref EditState editState, float minWidth, GUIStyle style, Func<string, string> formatter = null, params GUILayoutOption[] options) {
            var changed = false;
            if (editState.Item1 != label) {
                using (HorizontalScope(options.AddDefaults())) {
                    ...
                    if (GL.Button(Glyphs.Edit, GUI.skin.box, AutoWidth())) {
                        editState = new EditState(label, label);
                ...
                using (HorizontalScope(options)) {
                    string edit = editState.Item2;
                    TextField(ref edit, null, MinWidth(minWidth), AutoWidth());
                    Space(15);
                    if (GL.Button(Glyphs.CheckOff.red(), GUI.skin.box, AutoWidth())) {
                        editState = new EditState(null, null);
                    }
        ...
        public class EditState
        {
            ...
            public EditState(string item1, string item2)
            {
                Item1 = item1;
                Item2 = item2;
            }
        }
        public static bool EditableLabel(ref string label, ref EditState editState, float minWidth, Func<string, string> formatter = null, params GUILayoutOption[] options) => EditableLabel(ref label, ref editState, minWidth, GUI.skin.label, formatter, options);
