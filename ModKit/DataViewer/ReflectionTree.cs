...
namespace ModKit.DataViewer {
    ...
    public abstract class Node {
        ...
        public abstract IEnumerable<Node> GetItemNodes();
        public abstract IEnumerable<Node> GetComponentNodes();
        public abstract IEnumerable<Node> GetPropertyNodes();
        public abstract IEnumerable<Node> GetFieldNodes();
    ...
    internal abstract class GenericNode<TNode> : Node {
        ...
        public override IEnumerable<Node> GetComponentNodes() {
            UpdateComponentNodes();
            return _componentNodes.AsReadOnly();
        }
        public override IEnumerable<Node> GetItemNodes() {
            UpdateItemNodes();
            return _itemNodes.AsReadOnly();
        }
        public override IEnumerable<Node> GetFieldNodes() {
            UpdateFieldNodes();
            return _fieldNodes.AsReadOnly();
        }
        public override IEnumerable<Node> GetPropertyNodes() {
            UpdatePropertyNodes();
    ...
    internal class ComponentNode : PassiveNode<Component> {
        protected readonly WeakReference _parentNode;
        protected ComponentNode(Node parentNode, string name, Component value) : base(name, value, NodeType.Component) {
            _parentNode = new WeakReference(parentNode);
        }
        public override Node GetParent() {
            if (_parentNode != null && _parentNode.IsAlive)
            {
                return (Node)_parentNode.Target;
            }
            return null;
        }
    }
    ...
    internal abstract class ChildOfStructNode<TParent, TParentInst, TNode> : ChildNode<TParent, TNode>
        ...
        protected bool TryGetParentValue(out TParentInst value) {
            if (_parentNode != null && _parentNode.IsAlive)
            {
                var parent = (GenericNode<TParent>)_parentNode.Target;
                if (parent.InstType == typeof(TParentInst))
                {
