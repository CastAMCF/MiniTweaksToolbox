...
namespace ModKit.Utility {
    public static partial class ReflectionCache {
        ...
        private class CachedPropertyOfStruct<T, TProperty> : CachedProperty<TProperty> {
            ...
            public override TProperty Get() => (_getter ??= CreateGetter(typeof(Getter), Info.GetGetMethod(), true) as Getter)(ref _dummy);

            public TProperty Get(ref T instance) => (_getter ??= CreateGetter(typeof(Getter), Info.GetGetMethod(), true) as Getter)(ref instance);

            public override void Set(TProperty value) => (_setter ??= CreateSetter(typeof(Setter), Info.GetSetMethod(), true) as Setter)(ref _dummy, value);

            public void Set(ref T instance, TProperty value) => (_setter ??= CreateSetter(typeof(Setter), Info.GetSetMethod(), true) as Setter)(ref instance, value);
        }
        ...
        private class CachedPropertyOfClass<T, TProperty> : CachedProperty<TProperty> {
            ...
            public override TProperty Get() => (_getter ??= CreateGetter(typeof(Getter), Info.GetGetMethod(), false) as Getter)(_dummy);

            public TProperty Get(T instance) => (_getter ??= CreateGetter(typeof(Getter), Info.GetGetMethod(), false) as Getter)(instance);

            public override void Set(TProperty value) => (_setter ??= CreateSetter(typeof(Setter), Info.GetSetMethod(), false) as Setter)(_dummy, value);

            public void Set(T instance, TProperty value) => (_setter ??= CreateSetter(typeof(Setter), Info.GetSetMethod(), false) as Setter)(instance, value);
        }
        ...
        private class CachedPropertyOfStatic<TProperty> : CachedProperty<TProperty> {
            ...
            private Getter CreateGetter() => Delegate.CreateDelegate(typeof(Getter), Info.GetGetMethod()) as Getter;

            private Setter CreateSetter() => Delegate.CreateDelegate(typeof(Setter), Info.GetSetMethod()) as Setter;
        }
    }
}
